...
export function createRagTools(vectorStore: VectorStore | SqliteVectorStore, embedFn: EmbeddingFn): ToolDefinition[] {
    ...
    const ragSearchTool: ToolDefinition = {
        ...
        execute: async ({ query, top_k }: { query: string; top_k?: number }) => {
            ...
            const results = await hybridSearch(vectorStore as VectorStore, embedFn, query, top_k || 5);
