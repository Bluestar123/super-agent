...
// 1. mcp server  spawn 创建。 send 时候给 mcp server 的进程 stdin.write json。
// 2. mcp 接收处理后 ，输出， this.rl.on('line', xx, 监听接收，给回结果，主进程能拿到 index.ts
...
export class MCPClient {
  ...
  async connect(): Promise<void> {
    ...
    // 当前服务监听的 MCP 服务器输出，按照 JSON-RPC 格式解析响应
